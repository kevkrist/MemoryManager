...
private:
	...
	__host__ __forceinline__ void ScanSrcPairs(std::vector<SourcePair>& srcPairs, std::uint8_t* dest)
	{
		...
		// Scan the source pairs to determine the write offsets, starting block indices, sources, and update the srcPairs with the new src locations
		BlockStartsHost[0] = 0;
		SrcTriplesHost[0] = { srcPairs[0].Src, srcPairs[0].Count, 0 };
		srcPairs[0].Src = dest;
		for (auto i = 1; i < NumSources; ++i)
		{
			const auto writeOffset = GetAlignedCount(srcPairs[i - 1].Count) +
