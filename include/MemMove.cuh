...
private:
  ...
  __host__ __forceinline__ void ScanSrcPairs(std::vector<SourcePair>& srcPairs, std::uint8_t* dest)
  {
    ...
    // Scan the source pairs to determine the write offsets, starting block indices, sources
    BlockStartsHost[0] = 0;
    ...
    for (auto i = 1; i < NumSources; ++i)
    {
      ...
      SrcTriplesHost[i] = {srcPairs[i].Src,
                            srcPairs[i].Count,
                            GetAlignedCount(srcPairs[i - 1].Count) +
                              SrcTriplesHost[i - 1].WriteOffset};
    }
    NumTotalThreadBlocks = BlockStartsHost[NumSources - 1] +
                           GetNumThreadBlocks<BLOCK_THREADS>(srcPairs[NumSources - 1].Count);

    // Now insert the new sources into the srcPairs
    for (auto i = 0; i < NumSources; ++i)
    {
      srcPairs[i].Src = dest + SrcTriplesHost[i].WriteOffset;
    }
  }
