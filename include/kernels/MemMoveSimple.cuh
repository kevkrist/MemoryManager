...
__device__ __forceinline__ void BlockMemMove(
		std::uint8_t* dest,
...
)
{
	...
	auto num_tile_chunks =
			static_cast<std::int32_t>(CUB_MIN(num_chunks - offset, TILE_ITEMS));
	...
	else
	{
		...
		if (threadIdx.x < CUB_PTX_WARP_THREADS)
		{
			...
			tile_prefix_op_t prefix_op(scanTileState, prefix_storage, scan_op_t{});
...
)
{
	...
	BlockMemMove<BLOCK_THREADS, ITEMS_PER_THREAD>(
			dest,
			src,
			num,
			offset,
